            ...
            if row[0]:
                try:
                    if self.model == 'demographic':
                        self.good.append(Demographics(
                            **{'filename': self.file.split('\\')[-1], 'row_id': self.rows.index(row) + 1,
                               'id': "%04d" % id_index, 'date': format_date_time(row[3], 'datetime'),
                               'mem_id': row[4].upper(), 'cin': row[5].upper(), 'dob': format_date_time(row[7], 'date'),
                               'gender': get_gender(row[8]), 'last_name': row[9].title(), 'first_name': row[10].title(),
                               'middle_name': row[11].title(), 'email': row[12], 'opt_txt': yes_no(row[14]),
                               'opt_call': yes_no(row[15]),
                               'phone': {'home': re.sub('\D', '', row[16]), 'work': re.sub('\D', '', row[17]),
                                         'cell': re.sub('\D', '', row[13])},
                               'address': {'street': row[18], 'street2': row[19], 'city': row[20], 'state': row[21],
                                           'zip': row[22]}}))
                    elif self.model == 'enrollment':
                        self.good.append(Enrollment(
                            **{'filename': self.file.split('\\')[-1], 'row_id': self.rows.index(row) + 1,
                               'id': "%04d" % id_index, 'date': format_date_time(row[3], 'datetime'),
                               'mem_id': row[4].upper(), 'cin': row[5].upper(), 'next_visit': yes_no(row[7]),
                               'next_visit_date': format_date_time(row[8], 'date'),
                               'role': get_index(row[9], CONTACT_ROLE), 'role_other': row[10],
                               'contact_date': format_date_time(row[11], 'date'),
                               'effective_date': format_date_time(row[11], 'date'),
                               'term_date': format_date_time(row[12], 'date'), 'enrollment_flag': yes_no(row[14]),
                               'disenrollment_reason': get_disenrollment_key(row[13], DISENROLLMENT_REASON),
                               'udf': build_udfs(row, 15)}))
                    elif self.model == 'outreach':
                        self.good.append(Outreach(
                            **{'filename': self.file.split('\\')[-1], 'row_id': self.rows.index(row) + 1,
                               'id': "%04d" % id_index, 'date': format_date_time(row[1], 'datetime'),
                               'mem_id': row[2].upper(), 'cin': row[3].upper(), 'rescheduled': yes_no(row[5]),
                               'cancelled': yes_no(row[6]),
                               'appointment_date': format_date_time(row[7] + ' ' + row[8], 'datetime'),
                               'attempt_time': format_date_time(row[9] + ' ' + row[10], 'datetime'),
                               'attempt_result': get_index(row[11], ATTEMPT_RESULT),
                               'unsuccessful': get_index(row[12], ATTEMPT_UNSUCCESFUL_DISPOSITION),
                               'successful': get_index(row[13], ATTEMPT_SUCCESFUL_DISPOSITION),
                               'udf': build_udfs(row, 14)}))
                    id_index += 1
